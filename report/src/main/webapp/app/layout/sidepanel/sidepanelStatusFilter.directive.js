...
define(['angularAMD'], function (angularAMD) {
	'use strict';
	angularAMD.directive('aetSidepanelStatusFilter', ['$rootScope', '$timeout', SidepanelStatusFilterDirective]);
		...
		function onFilterLabelClick() {
			if (!_.isEmpty($('.filters .popover'))) {
				...
				$rootScope.activeFilters.forEach(function (filter) {
					$popoverContent.find('p[data-attribute="' + filter + '"]').siblings().prop('checked', ' ');
				});
			}
		}

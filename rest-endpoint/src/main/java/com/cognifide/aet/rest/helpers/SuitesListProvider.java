...
public class SuitesListProvider {
  ...
  public String listSuites() {
    ...
    try {
      ...
      sb.append("<h1>Total ").append(projects.size()).append(" projects are present in database.</h1><hr>");
      for (DBKey dbKey : projects) {
        ...
        sb.append(" has total <b>").append(uniqueSuites.keySet().size()).append("</b> unique suites.");
        ...
        for (Map.Entry<String, Collection<Suite>> suiteEntries : uniqueSuites.asMap().entrySet()) {
          ...
          for (Suite suite : suiteEntries.getValue()) {
            ...
            sb.append(" runned at ").append(simpleDateFormat.format(new Date(suite.getRunTimestamp().get())));
            sb.append("</br>&nbsp;&nbsp;&nbsp; ")

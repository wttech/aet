...
public class JsErrorsComparator implements ComparatorJob {
  ...
  JsErrorsComparator(ComparatorProperties comparatorProperties, List<DataFilterJob> dataFilterJobs, ArtifactsDAO artifactsDAO) {
  ...
  public ComparatorStepResult compare() throws ProcessingException {
    ...
    try {
      ...
      if (jsErrorLogs.isEmpty()) {
        ...
        final String resultArtifactId = artifactsDAO.saveArtifactInJsonFormat(comparatorProperties, jsErrorLogs);
  ...
  private Set<JsErrorLog> getCollectedResult() throws IOException {
    return artifactsDAO.getJsonFormatArtifact(comparatorProperties, comparatorProperties.getCollectedId(),
            RESULT_TYPE);
  }

...
public class SuitesRemovePredicateProcessor implements Processor {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(SuitesRemovePredicateProcessor.class);

  @Override
  @SuppressWarnings("unchecked")
  public void process(Exchange exchange) throws Exception {
    final AllSuiteVersionsMessageBody allSuiteVersions = exchange.getIn().getBody(AllSuiteVersionsMessageBody.class);
    ...
    LOGGER.info("Processing suite `{}` with {} version(s) in {}", allSuiteVersions.getData(), suiteVersions.size(), dbKey);

    final SuiteRemoveCondition removeCondition = new SuiteRemoveCondition(suiteVersions, cleanerContext);
    ...
    exchange.getOut().setHeader(SuiteAggregationCounter.NAME_KEY, new SuiteAggregationCounter(suiteVersions.size()));

...
public class XmlReporterJobHandler extends BasicReporterJobHandler {
	...
	protected ReportResult createResultData(ReporterHandlerWorkload workload, List<Node> resultNodes,
		...
		return XmlReportJobBuilder.create()
				.withResultsNodes(resultNodes)
				.withExpectedResults(workload.getJob().getExpectedResults())
				.withReportsProperties(reportsProperties)
				.withServiceUrl(reportUrl)
				.build();
	}

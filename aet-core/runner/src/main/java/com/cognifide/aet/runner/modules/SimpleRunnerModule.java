...
public class SimpleRunnerModule extends AbstractModule {
	...
	public SimpleRunnerModule(long failureTimeout, long messageTimeToLive, int urlPackageSize,
			int maxMessagesInCollectorQueue, VersionStorage versionStorage, CommentsManager commentsManager,
			JmsConnection jmsConnection, MessagesManager messagesManager, RunnerMode runnerMode) {
	...
	protected void configure() {
		...
		bind(VersionStorage.class).toInstance(versionStorage);

		bind(CommentsManager.class).toInstance(commentsManager);
		...
		bind(TestSuiteRun.class).toProvider(TestSuiteScope.<TestSuiteRun> seededKeyProvider()).in(

...
public class TestSuiteTask implements Runnable {
	...
	public TestSuiteTask(Provider<TestLifeCycle> testLifeCycleProvider,
			...
			@Assisted TestSuiteRun testSuiteRun, @Assisted Destination resultsDestination,
	...
	public void run() {
		scope.enter();
		try {
			scope.seed(Key.get(TestSuiteRun.class), testSuiteRun);
			...
			LOGGER.error("Error while starting TestLifeCycle for: {}.", testSuiteRun.getCorrelationId(), e);
		} finally {
			scope.exit();
		}
	}

...
public class AccessibilityCollector implements CollectorJob {
	...
	public AccessibilityCollector(
			Node dataNode, CollectorProperties properties, WebDriver webDriver,
	...
	public boolean collect() throws ProcessingException {
		...
		final String html = (String) ((JavascriptExecutor) webDriver).executeScript("return document.documentElement" +
				".outerHTML;");
		...
		try {
			AccessibilityCollectorResult collectorResult = new AccessibilityCollectorResult(url, issues);
			dataNode.saveResult(collectorResult);

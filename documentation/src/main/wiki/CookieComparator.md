...
| --------- | ----- | ----------- | --------- |
| `action` | list<br/><br/> test<br/><br/> compare | Displays a list of cookies<br/><br/> Tests if a cookie with a given name and value exists<br/><br/> Compares current data to the pattern (compares only cookie names, values are ignored) | no<br/><br/> If the `action` parameter is not provided, the default `list` action is performed |
